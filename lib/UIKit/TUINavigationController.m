...
- (id)initWithRootViewController:(TUIViewController *)viewController {
	...
	_stack = @[viewController].mutableCopy;
...
- (void)loadView {
	...
	TUIViewController *visible = [self topViewController];  [visible viewWillAppear:NO];
	if ([self.delegate respondsToSelector:@selector(navigationController:willShowViewController:animated:)]) [self.delegate navigationController:self willShowViewController:visible animated:NO];
	[self.view addSubview:visible.view];    visible.view.frame = self.view.bounds;
	visible.view.autoresizingMask = TUIViewAutoresizingFlexibleSize;
	[visible viewDidAppear:YES];
	if ([self.delegate respondsToSelector:@selector(navigationController:didShowViewController:animated:)]) [self.delegate navigationController:self didShowViewController:visible animated:NO];
}
...
- (void)setViewControllers:(NSArray *)viewControllers animated:(BOOL)animated completion:(void (^)(BOOL finished))completion {
	...
	viewController.view.frame = (containedAlready ? TUINavigationOffscreenLeftFrame(self.view.bounds) :
	                             TUINavigationOffscreenRightFrame(self.view.bounds));
	[CATransaction flush];
	[CATransaction commit];
	TUIViewController *last = [self topViewController];
	for (TUIViewController *controller in _stack) controller.navigationController = nil;
	[_stack removeAllObjects];
	[_stack addObjectsFromArray:viewControllers];
	for (TUIViewController *controller in viewControllers) controller.navigationController = self;
	[TUIView animateWithDuration:duration animations:^{
	    last.view.frame = (containedAlready ? TUINavigationOffscreenRightFrame(self.view.bounds)
						   : TUINavigationOffscreenLeftFrame(self.view.bounds));
	    viewController.view.frame = self.view.bounds;
	} completion:^(BOOL finished) {
	    [last.view removeFromSuperview];
	    [viewController viewDidAppear:animated];
	    if ([self.delegate respondsToSelector:@selector(navigationController:didShowViewController:animated:)]) [self.delegate navigationController:self didShowViewController:viewController animated:animated];
...
- (void)pushViewController:(TUIViewController *)viewController animated:(BOOL)animated completion:(void (^)(BOOL finished))completion {
	...
	if ([self.delegate respondsToSelector:@selector(navigationController:willShowViewController:animated:)]) [self.delegate navigationController:self willShowViewController:viewController animated:animated];
	[self.view addSubview:viewController.view];
...
//            [CATransaction setAnimationTimingFunction:[CAMediaTimingFunction functionWithControlPoints: 0:1 : 1:1]];
	    ...
	    last.view.layer.filters = nil;                          [last.view removeFromSuperview];
	    viewController.view.layer.filters = nil;        [viewController viewDidAppear:animated];
	    if ([self.delegate respondsToSelector:@selector(navigationController:didShowViewController:animated:)]) [self.delegate navigationController:self didShowViewController:viewController animated:animated];
	    [last viewDidDisappear:animated];
	    if (completion) completion(finished);
	}];
